...
//将内核事件表注册读事件，ET模式(边缘触发，一次性读完所有数据），选择开启EPOLLONESHOT
//向epoll中添加需要监听的文件描述符
void addfd(int epollfd, int fd, bool one_shot, int TRIGMode)
{
    ...
    //对端连接断开触发的epoll事件会包含epollin|epollrdhup
    //有了这个事件，对端断开连接的异常就可以在底层处理，不用移交到上层
    if (1 == TRIGMode)
...
//解析http请求首行，获得请求方法，目标url及http版本号
...
//没有真正解析HTTP请求的消息体，只是判断http请求是否被完整读入
    ...
    {   //分散写
        ...
        if (temp < 0)
        {
            ...
            {   //如果TCP写缓冲没有空间，则等待下一轮EPOLLOUT事件，虽然在此期间
                //服务器无法立即接收到同一客户的下一个请求，但是可以保证连接的完整
